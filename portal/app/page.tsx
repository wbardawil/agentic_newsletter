...
export default async function HomePage() {
  ...
  const { data: latest } = await supabase
    .from("editions")
    .select("edition_id, edition_number, subject_en, subject_es, shareable_sentence_en, shareable_sentence_es, pillar, published_at")
  ...
  return (
    <>
      ...
      <section className="container-wide py-16 border-t border-[var(--color-line)]">
        <h2 className="text-3xl mb-10">{i18n.pillarsHeading}</h2>
...
function Pillar({ order, title, body }: { order: string; title: string; body: string }) {
  return (
    <div className="card">
      ...
      <p className="text-[var(--color-bronze)]">{body}</p>
    </div>
  );
}
